...
export const getDonors = (req: Request, res: Response) => {
    UserDeprecated.find({ donorInfo: { $exists: true } })
...
export const postDonors = (req: Request, res: Response) => {
    const donor = new UserDeprecated(req.body);
    return donor.save()
        .then(result => {
...
export const modifyDonor = (req: Request, res: Response) => {
    ...
    return UserDeprecated.findByIdAndUpdate(id, req.body)
        .then(result => {
...
export const getDonorDetails = (req: Request, res: Response) => {
    ...
    const id = req.params.donor_id;
    return UserDeprecated.find({ _id: id, donorInfo: { $exists: true } }, 'donorInfo.name donorInfo.phone donorInfo.address donorInfo.latitude donorInfo.longitude')
...
export const postDonations = (req: Request, res: Response) => {
    ...
    UserDeprecated.findOne({ sub: { $eq: payload.sub } }).then(user => {
        console.log(user);
        ...
        try {
            if ((!req.files || !req.files.descriptionImage) && !jsonBody.description) {
                ...
                UserDeprecated.findById(jsonBody.donor).then(result => {
